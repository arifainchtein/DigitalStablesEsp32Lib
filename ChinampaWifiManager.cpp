...
ChinampaWifiManager::ChinampaWifiManager(HardwareSerial &serial,  FS &fs,PCF8563TimeManager &t, Esp32SecretManager &e,  ChinampaData& tf,ChinampaConfigData& p) : 
WifiManager(serial ,fs,  t,e) , chinampaData(tf),chinampaConfigData(p){}

 void ChinampaWifiManager::setWifiActiveSwitchStatus(bool b){
    wifiActiveSwitchStatus=b;
 }

void ChinampaWifiManager::start(){
   ...
   ssid = secretManager.getSSID();
    ...
    _HardSerial.print("in  ChinampaWifiManager ssid=");
...
asyncWebServer.on("/assets/img/Chinampa.svg", HTTP_GET, [this](AsyncWebServerRequest *request){
    this->_HardSerial.println("request Chinampa.svg");
    request->send(_fs, "/Chinampa.svg", String(), false);
    delay(5);
});
...
asyncWebServer.on("/index.html", HTTP_GET, [this](AsyncWebServerRequest *request){
    this->_HardSerial.println("requesting index.html=");
    request->send(_fs, "/index.html", String(), false);
    delay(5);
});

asyncWebServer.on("/assets/css/Roboto.css", HTTP_GET, [this](AsyncWebServerRequest *request){
    this->_HardSerial.println("requesting robottocss=");
...
asyncWebServer.on("/ChinampaServlet", HTTP_POST, [this](AsyncWebServerRequest *request) {
    ...
    if(formName=="ConfigSTA"){
      ...
      chinampaData.latitude=lat;
      chinampaData.longitude=log;
...
asyncWebServer.on("/ChinampaServlet", HTTP_GET, [this](AsyncWebServerRequest *request) {
    int paramsNr = request->params();
    this->_HardSerial.println(paramsNr);
    const AsyncWebParameter* p = request->getParam(1);
...
void ChinampaWifiManager::generateWebData(DynamicJsonDocument& json, String sentBy){
    
    json["devicename"] = chinampaData.devicename;
    json["deviceshortname"] = chinampaData.deviceshortname;
    json["groupidentifier"]=chinampaData.groupidentifier;
    ...
    json["fishtankoutflowflowRate"]=chinampaData.fishtankoutflowflowRate;

    json["minimumSumpTroughLevel"] = chinampaData.minimumSumpTroughLevel;
    json["maximumSumpTroughHemaximumSumpTroughLevelight"] = chinampaData.maximumSumpTroughLevel;
...
int ChinampaWifiManager::uploadDataToDigitalStables(){
  ...
  _HardSerial.println(httpResponseCode);
   chinampaData.digitalStablesUpload=false;
   chinampaData.internetAvailable=false;
    digitalWrite(WATCHDOG_WDI, HIGH);
    delay(2);
    digitalWrite(WATCHDOG_WDI, LOW);
  if (httpResponseCode == 200) { //Check for the returning code
      ...
      if(response=="Ok"){
        toReturn =true;
         chinampaData.digitalStablesUpload=true;
          chinampaData.internetAvailable=true;
      }else{
